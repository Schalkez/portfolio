...
onmessage = function (e) {
  ...
  if (type === "createGeometry") {
    ...
    let oceanMorphPositionsBuffer, oceanMorphNormalsBuffer;
    if (
      oceanGeometry.morphAttributes.position &&
      oceanGeometry.morphAttributes.position[0]
    ) {
      oceanMorphPositionsBuffer =
        oceanGeometry.morphAttributes.position[0].array.buffer;
    }
    if (
      oceanGeometry.morphAttributes.normal &&
      oceanGeometry.morphAttributes.normal[0]
    ) {
      oceanMorphNormalsBuffer =
    ...
    postMessage(
      {
        ...
        data: {
          ...
          oceanNormals,
          vegetation,
          oceanMorphPositions: oceanMorphPositionsBuffer,
          oceanMorphNormals: oceanMorphNormalsBuffer,
        },
        requestId,
      },
      // @ts-expect-error - hmm
